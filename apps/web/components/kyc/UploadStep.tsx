...
export function UploadStep({
  ...
  const handleDigiLockerAuth = async () => {
    try {
      ...
      const { authorizationUrl } = await initiateDigiLockerAuth(
        submissionId
      );
  ...
  const handleFetchFromDigiLocker = async () => {
    try {
      ...
      if (res.documentsFetched.includes("PAN")) {
        dispatch(setPanUploaded(true));
        onStateRestored?.(2); // move to next step
      } else {
        ...
        <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-xl border border-gray-100 flex flex-col items-center text-center">
          ...
          {!digiLockerAuthorized ? (
            ...
            <button
              ...
              className="w-full py-3.5 px-4 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-medium shadow-lg shadow-teal-200 hover:shadow-teal-300 hover:scale-[1.02] transition-all duration-200 disabled:opacity-70 disabled:hover:scale-100 flex items-center justify-center gap-2"
            >
              {fetchingFromDigiLocker ? (
                <>
                  <svg className="animate-spin -ml-1 mr-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                  Fetching details securely...
                </>
              ) : (
                <>
                  Fetch PAN Identity from Vault
                </>
              )}
            </button>
          )}
