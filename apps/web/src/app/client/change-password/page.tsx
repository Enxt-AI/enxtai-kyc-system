 ...
 *
 * Handles forced first-login password reset and voluntary password changes for client users.
 *
 * @remarks
 * **Forced First-Login Flow**:
 * - Triggered when `session.user.mustChangePassword === true`
 * - ClientRoleGuard redirects to this page before allowing dashboard access
 * - User must change password to proceed to portal
 * - After successful change, `mustChangePassword` is set to `false`
 *
 * **Voluntary Change Flow**:
 * - User clicks "Change Password" link in navigation
 * - No redirect enforcement (can cancel and return to dashboard)
 * - Success message shown, user can stay on page or navigate away
 *
 ...
 * - Isolated auth context via ClientSessionProvider
 *
 * **API Integration**:
 ...
 * - Trigger session refresh (updates mustChangePassword=false)
 * - Redirect to `/client/dashboard` (forced flow)
 * - Stay on page with success message (voluntary flow)
 */
function ChangePasswordForm() {
  ...
  /**
   * Handle manual redirect via "Okay" button.
   * Clears countdown timer and navigates to dashboard immediately.
   */
